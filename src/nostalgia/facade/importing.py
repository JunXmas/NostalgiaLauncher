...
from nostalgia.errors import ContentError, InstanceError
from nostalgia.facade.modpacks import ModpackOperations
from nostalgia.importing.launchers import Found
from nostalgia.importing.worlds import copy_worlds
...
class ImportOperations(ModpackOperations):
    ...
    def sync_from_room_code(
        ...
        *,
        ...
        """Dán mã phòng → kết nối relay → lấy manifest → diff → tải → tạo instance.

        Chạy vòng lặp asyncio tạm thời vì facade là synchronous, nhưng giao thức relay
        là async. Dùng `asyncio.run` cho trường hợp này.
        """
        import asyncio

        from nostalgia.multiplayer.room_code import normalise_room_code
        from nostalgia.multiplayer.rooms_guest import build_guest_instance, probe_room

        self.require_instance_id_free(instance_id)

        normalised = normalise_room_code(room_code)

        on_progress(Progress(stage="Kết nối phòng", done=0, total=4))

        # Probe để lấy thông tin phòng.
        relay_url = self.endpoints.multiplayer_relay
        room_info = asyncio.run(probe_room(relay_url, normalised))

        on_progress(Progress(stage="Tạo bản chơi", done=1, total=4))

        # Tạo instance với thông tin từ host.
        version_id = room_info.game_version
        if room_info.loader_kind != "vanilla" and room_info.game_version:
            candidates = [
                installed_id
                for installed_id in self.list_installed_versions()
                if detect_loader_kind(installed_id) == room_info.loader_kind
                and installed_id.endswith(room_info.game_version)
            ]
            if candidates:
                version_id = max(candidates)

        if not version_id:
            message = "host không gửi phiên bản game"
            raise ContentError(message)
        ...
        instance = self.create_instance(
            Instance(
                instance_id=instance_id,
                version_id=version_id,
                display_name=display_name or f"Rooms – {room_info.host_name}",
                game_dir_override=game_dir_override,
            )
        )
        target_game_dir = self.instance_game_dir(instance)

        on_progress(Progress(stage="Đồng bộ mods", done=2, total=4))
        ...
        on_progress(Progress(stage="Hoàn tất", done=4, total=4))
        return instance
