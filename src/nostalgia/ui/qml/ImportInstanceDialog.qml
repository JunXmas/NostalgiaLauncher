...
/* Hộp thoại nhập bản chơi — ba tab: file .mrpack, launcher khác, đồng bộ Rooms.
...
Item {
    ...
    Rectangle {
        ...
        Item {
            ...
            // Tab 2: Đồng bộ Rooms
            Column {
                visible: dialog.currentTab === 2
                anchors { left: parent.left; right: parent.right }
                spacing: 16

                Text { text: "Nhập mã phòng từ người chơi khác để đồng bộ mods và config"; color: Theme.textMuted; font.pixelSize: 12 }
                ...
                Row {
                    ...
                    ActionButton {
                        label: importBridge.busy ? "Đang đồng bộ..." : "Đồng bộ"
                        clickable: !importBridge.busy && roomCodeField.text.length > 0
                        onClicked: importBridge.syncFromRoomCode(roomCodeField.text, "")
                    }
                }

                // Nút dán từ clipboard
                ActionButton {
                    primary: false; label: "Dán mã từ clipboard"
                    clickable: !importBridge.busy
                    onClicked: {
                        var clip = multiplayerBridge.clipboardText();
                        if (clip.length > 0) roomCodeField.text = clip;
                    }
                }
