...
class ImportBridge(WorkerBridge):
    ...
    @Slot(str, str)
    def syncFromRoomCode(self, room_code: str, display_name: str) -> None:
        """Đồng bộ bản chơi từ mã phòng Rooms."""
        room_code = room_code.strip()
        if not room_code:
            self.importError.emit("Nhập mã phòng trước")
            return

        def work() -> None:
            taken = {i.instance_id for i in self._launcher.list_instances()}
            display_label = display_name.strip() or f"Rooms-{room_code[:6]}"
            instance = self._launcher.sync_from_room_code(
                room_code,
                slugify(display_label, taken),
                display_label,
                on_progress=self._main_bridge.report_progress,
        ...
        self.run_in_background(work, f"Đồng bộ từ phòng {room_code[:6]}...")
